...
def parsing_recv_packets(pkt):
    try:
        if IP in pkt and pkt[IP].tos != 0x3 and TCP in pkt and pkt[TCP].load:
            ...
            # EXTRACT SEQUENCE NUMBER FROM TCP SOURCE PORT
            # Subtract 10000 to get a clean 0, 1, 2, 3... index
            seq_num = pkt[TCP].sport - 10000 
            
            if FatIntHdr.case == 0 or FatIntHdr.case == 1:
                # Pass seq_num instead of the absolute timestamp
                parse_metadata_q(pkt, FatIntHdr.queue_space, seq_num)
                parse_metadata_hop(pkt, FatIntHdr.hop_space, seq_num)
                parse_metadata_egress(pkt, FatIntHdr.egress_space, seq_num)
